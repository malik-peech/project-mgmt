...
const UPSELLS_TABLE_ID = 'tbldmU1cSz2n66N8C'
const TMP_DIR = '/tmp/pm-uploads'

const FIELD_NAME = 'Devis / BDC Upsell'

/**
 * POST /api/upsells/[id]/upload
 * Upload one or more files to the "Devis / BDC Upsell" attachment field.
 */
export async function POST(
  ...
  try {
    ...
    const getRes = await fetch(
      `https://api.airtable.com/v0/${BASE_ID}/${UPSELLS_TABLE_ID}/${recordId}`,
      { headers: { Authorization: `Bearer ${apiKey}` } }
    )
    let existingAttachments: { id?: string; url: string; filename?: string }[] = []
    if (getRes.ok) {
      const record = await getRes.json()
      existingAttachments = (record.fields?.[FIELD_NAME] as { id?: string; url: string; filename?: string }[]) || []
    }
    ...
    if (!updateRes.ok) {
      ...
      console.error('[Upsell upload] Airtable error:', updateRes.status, err)
    ...
    try {
      ...
      upsertRecord(TABLES.UPSELLS, { id: updated.id, fields: updated.fields })
    } catch (e) {
      console.error('[Upsell upload] failed to parse response:', e)
    }

    return NextResponse.json({ ok: true, count: newAttachments.length, fields: updatedFields })
  } catch (error) {
    console.error('Error uploading upsell attachment:', error)
    return NextResponse.json({ error: 'Failed to upload' }, { status: 500 })
  }
}

/**
 * DELETE /api/upsells/[id]/upload?attachmentId=xxx | ?attachmentUrl=xxx
 * Remove a single attachment from the "Devis / BDC Upsell" field.
 */
export async function DELETE(
  ...
  try {
    ...
    const record = await getRes.json()
    const existing = (record.fields?.[FIELD_NAME] as { id: string; url?: string }[]) || []
    ...
    console.error('Error deleting upsell attachment:', error)
